...
public final class WantedSessionManager implements Listener {
    ...
    public boolean canTriggerCrime(Player player, Crime crime){
        Settings.RearmMode mode = plugin.settings().rearmMode();
        if(mode==Settings.RearmMode.NONE) return true;
        RearmInfo info = rearm.get(player.getUniqueId());
    ...
    public void startOrRefreshSession(Player player, Location anchor, Crime crime){
        ...
        if(plugin.settings().refreshPolicy()==Settings.RefreshPolicy.ANY_TRIGGER){ s.refreshAndRetarget(anchor, crime); } else { s.refresh(); }
    }
    ...
    private final class Session{
        ...
        \1
        final Set<UUID> prevObservers = new HashSet<>();
        boolean ended = false;
...
private void refreshObservers(){
    ...
    for(UUID id : old){
        ...
        void refreshAndRetarget(Location newAnchor, Crime newCrime){
            anchorLoc = newAnchor==null?null:newAnchor.clone();
            if(newCrime!=null) crime = newCrime;
            refreshObservers();
            cleanupEntities();
            Player p=Bukkit.getPlayer(playerId);
            if(p==null || !p.isOnline()){ end("player-offline"); return; }
            if(phase==Phase.WITNESS){ startWitness(p); }
            else{
                markNearbyVillagersHostile(p);
                int naturalNearby=countNaturalHostilesNear(p)-1;
                int extras=0; if(plugin.settings().witnessEnabled()){ if(naturalNearby==1) extras=4; else if(naturalNearby==2) extras=3; else if(naturalNearby==3) extras=2; }
                if(extras>0) spawnEnforcersAround(p,extras);
                secondsTotal=plugin.settings().durationSeconds(); endTimeMs=System.currentTimeMillis()+secondsTotal*1000L;
                plugin.displayManager().showBossbar(p, amp(plugin.settings().wantedBossbarText()), plugin.settings().bossbarColor(), plugin.settings().bossbarOverlay(), 1.0f);
                scheduleNextTickWanted();
            }
        }
        ...
        private void scheduleNextTickWitness(){ cancelTask();
            sessions.remove(playerId, this);
            refreshObservers(); int i=Math.max(2, Math.min(10, plugin.settings().logicTickTicks()));
            taskId=Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, this::tickWitness, i); }
        private void scheduleNextTickWanted(){ cancelTask();
            sessions.remove(playerId, this);
            refreshObservers(); int i=Math.max(2, plugin.settings().logicTickTicks());
    ...
    // ===================== Listeners =====================
    private final class ProjectileListener implements Listener {
        @EventHandler(ignoreCancelled=true)
        public void onHit(ProjectileHitEvent e){
            if(!(e.getEntity() instanceof Snowball ball)) return;
            if(!(ball.getShooter() instanceof Villager villager)) return;

            // Villager friendly fire immunity
            if(plugin.settings().preventVillagerFriendlyFire()){
                if(e.getHitEntity() instanceof Villager){ e.setCancelled(true); return; }
            }
    ...
    private final class WorldListener implements Listener {
        @EventHandler(ignoreCancelled=true)
        public void onVillagerDeathWitness(EntityDeathEvent e){
            if(!(e.getEntity() instanceof Villager v)) return;
            // Match by session witness UUID (PDC may be cleared earlier)
            Session owner=null;
            for(Session s : sessions.values()){
                if(s.witnessVillager!=null && v.getUniqueId().equals(s.witnessVillager)){ owner=s; break; }
            }
            if(owner==null) return;
            if(plugin.settings().debug()) plugin.getLogger().info("[Wanted] Witness villager eliminated -> ending witness pre-event.");
        ...
        public void onVillagerDeathMurder(EntityDeathEvent e){
            ...
            if(isParticipant(killer)) return; // already inside an event

            // Don't trigger if villager is already hostile/witness
            Byte hostile=v.getPersistentDataContainer().get(plugin.keyHostileVillager(), PersistentDataType.BYTE);
            ...
            int nearby=0;
            for(Entity ent : v.getNearbyEntities(plugin.settings().radius(), plugin.settings().radius(), plugin.settings().radius())){
        ...
        public void onVillagerDamage(EntityDamageByEntityEvent e){
            // Block villager vs villager projectile friendly fire
